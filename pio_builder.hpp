...
public:
	...
	constexpr uint8_t jmp(
	    const JmpCond cond, const uint8_t addr = 0, const uint8_t delay = 0,
	    const uint8_t side_set = 0, const bool side_set_en = 1) {
	...
	constexpr uint8_t wait(
	    const bool polarity, const WaitSrc src, const uint8_t index, const uint8_t delay = 0,
	    const uint8_t side_set = 0, const bool side_set_en = 1) {
	...
	constexpr uint8_t in(
	    const InSrc src, const uint8_t bit_count, const uint8_t delay = 0,
	    const uint8_t side_set = 0, const bool side_set_en = 1) {
		return in_at(current_idx++, src, bit_count, delay, side_set, side_set_en);
	}

	constexpr uint8_t in_at(
	    const uint8_t instr_idx, const InSrc src, const uint8_t bit_count, const uint8_t delay = 0,
	    const uint8_t side_set = 0, const bool side_set_en = 1) {
		auto src_u8 = static_cast<uint8_t>(src);
	...
	constexpr uint8_t out(
	    const OutDest dest, const uint8_t bit_count, const uint8_t delay = 0,
	    const uint8_t side_set = 0, const bool side_set_en = 1) {
	...
	constexpr uint8_t push(
	    const bool block = 1, const bool if_full = 0, const uint8_t delay = 0,
	    const uint8_t side_set = 0, const bool side_set_en = 1) {
	...
	constexpr uint8_t mov(
	    const MovDest dest, const MovSrc src, const MovOp op = MovOp::None, const uint8_t delay = 0,
	    const uint8_t side_set = 0, const bool side_set_en = 1) {
	...
	constexpr uint8_t irq(
	    const uint8_t index, const bool clear = 0, const bool wait = 0, const uint8_t delay = 0,
	    const uint8_t side_set = 0, const bool side_set_en = 1) {
	...
	constexpr uint8_t set(
	    const SetDest dest, const uint8_t data, const uint8_t delay = 0, const uint8_t side_set = 0,
	    const bool side_set_en = 1) {
		return set_at(current_idx++, dest, data, delay, side_set, side_set_en);
	}

	constexpr uint8_t set_at(
	    const uint8_t instr_idx, const SetDest dest, const uint8_t data, const uint8_t delay = 0,
	    const uint8_t side_set = 0, const bool side_set_en = 1) {
		auto dest_u8 = static_cast<uint8_t>(dest);
...
private:
	constexpr void write_instr(
	    const uint8_t instr_idx, const uint16_t instr, const uint8_t delay = 0,
	    const uint8_t side_set = 0, const bool side_set_en = 1) {
